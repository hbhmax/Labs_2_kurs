...
const char* error_messages[] = {
    [SUCCESS] = "Success",
    [ERROR_ZECKENDORF_NEGATIVE] = "Zeckendorf representation is only for non-negative numbers",
    [ERROR_BASE_OUT_OF_RANGE] = "Base out of range [2..36]",
    [ERROR_INVALID_NUMBER_FORMAT] = "Invalid number format for the given base",
    [ERROR_INVALID_CHARACTER] = "Invalid character in number representation",
    [ERROR_EMPTY_STRING] = "Empty string provided",
    [ERROR_INVALID_CASE] = "Invalid letter case for the specified format"
};

const char* get_error_message(ErrorCode error) {
    if (error >= SUCCESS && error <= ERROR_INVALID_CASE) {
...
typedef struct {
    ...
    int arg_count;
    union {
        ConversionResult (*converter1)(int);
        ConversionResult (*converter2)(int, int);
        ConversionResult (*converter3)(char*, int);
        ConversionResult (*converter4)(unsigned int);
        ConversionResult (*converter5)(double);
        ConversionResult (*converter6)(float);
    } conv;
} CustomFlag;
...
ConversionResult task_a(unsigned int num) {
    ...
    for (int i = 0; i < ROMAN_COUNT; i++) {
        while (num >= roman_values[i]) {
            strcat(cr.result, roman_symbols[i]);
            num -= roman_values[i];
        }
    }
...
ConversionResult task_b(int num) {
    ...
    if (num < 0) {
        cr.error = ERROR_ZECKENDORF_NEGATIVE;
        return cr;
    }
...
ConversionResult task_c(int num, int base) {
    ...
    if (base < 2 || base > 36) {
        cr.error = ERROR_BASE_OUT_OF_RANGE;
        return cr;
    }
    ...
    int result_index = 0;
    for (int i = index - 1; i >= 0; i--) {
        cr.result[result_index++] = buffer[i];
    }
    cr.result[result_index] = '\0';
...
ConversionResult task_e(char *num, int base) {
    ...
    for (; num[i] != '\0'; i++) {
        ...
        if (c >= '0' && c <= '9') {
            ...
            cr.error = ERROR_INVALID_CASE;
            return cr;
        } else {
            cr.error = ERROR_INVALID_CHARACTER;
            return cr;
        }
        
        if (digit >= base) {
            cr.error = ERROR_INVALID_NUMBER_FORMAT;
            return cr;
        }
...
ConversionResult task_g(int num) {
    ConversionResult cr;
    dump_memory(&num, sizeof(int), cr.result);
    cr.error = SUCCESS;
    return cr;
}
...
ConversionResult task_h(unsigned int num) {
    ConversionResult cr;
    dump_memory(&num, sizeof(unsigned int), cr.result);
    cr.error = SUCCESS;
    return cr;
}
...
ConversionResult task_i(double num) {
    ConversionResult cr;
    dump_memory(&num, sizeof(double), cr.result);
    cr.error = SUCCESS;
    return cr;
}
...
ConversionResult task_j(float num) {
    ConversionResult cr;
    dump_memory(&num, sizeof(float), cr.result);
    cr.error = SUCCESS;
    return cr;
}
...
int process_format(const char *format, va_list args, char *buffer, int bufsize, int *has_error) {
    int pos = 0;
    *has_error = 0;
        ...
        if (format[i] == '%') {
            ...
            if (!matched) {
                ...
                if (strcmp(specifier, "%d") == 0 || strcmp(specifier, "%i") == 0) {
                    ...
                    double arg = va_arg(args, double);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else if (strcmp(specifier, "%lf") == 0) {
                    double arg = va_arg(args, double);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else if (strcmp(specifier, "%p") == 0) {
                    void* arg = va_arg(args, void*);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else if (strcmp(specifier, "%x") == 0 || strcmp(specifier, "%X") == 0) {
                    unsigned int arg = va_arg(args, unsigned int);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else if (strcmp(specifier, "%o") == 0) {
                    unsigned int arg = va_arg(args, unsigned int);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else if (strcmp(specifier, "%g") == 0) {
                    double arg = va_arg(args, double);
                    snprintf(temp, sizeof(temp), specifier, arg);
                } else {
...
int overfprintf(FILE *stream, const char *format, ...) {
    ...
    char buffer[4096];
    int has_error = 0;
    int result = process_format(format, args, buffer, sizeof(buffer), &has_error);
