...
TEST(MemoryResourceTest, MemoryReuse) {
    ...
    // Снова выделяем - должен переиспользовать память
...
// Тесты для forward_list с простыми типами
TEST(ForwardListTest, BasicOperationsInt) {
    ...
    // Проверка содержимого через итератор
    auto it = list.begin();
    EXPECT_EQ(*it, 3);
    ++it;
    EXPECT_EQ(*it, 2);
...
// Тесты для forward_list со сложными типами
